...
export default function DuplicadosPage() {
  ...
  useEffect(() => {
    if (loaded && (!staff || !veTudo(staff.role))) router.replace(staff ? '/painel/condominios' : '/painel/login');
  }, [loaded, staff, router]);
  const carregar = () =>
    listarDuplicados()
      .then((d) => {
                  ...
                  return (
                    <div key={c.id} className={`flex flex-col gap-1.5 rounded-xl border p-3 text-sm ${ehPrincipal ? 'border-accent ring-1 ring-accent' : 'border-[var(--border)]'}`}>
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-3 text-xs font-semibold">
                        ...
                        <a href={`/painel/condominios/${c.id}/editar`} target="_blank" rel="noreferrer" className="text-accent hover:underline">
                          abrir
                        </a>
                      </div>
                    </div>
